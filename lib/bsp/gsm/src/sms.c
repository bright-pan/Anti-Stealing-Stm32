 ...
 * Modified at:   Thu May  5 14:44:13 2011
 *                
 * Description:   
...
/* 邮件时间解析格式为20XX-XX-XX,XX:XX:XX*/
	...
	*UCS++ = 0X3200;// 2;
	*UCS++ = 0X3000;// 0;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.year & 0xf0) << 4) | 0x3000;//解析年的高四为年的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.year & 0x0f) << 8) | 0x3000;//解析年的高四为年的个位;
	*UCS++ = YEAR;// 年;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.month & 0xf0) << 4) | 0x3000;//解析年的高四为月的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.month & 0x0f) << 8) | 0x3000;//解析年的高四为月的个位;
	*UCS++ = MONTH;// 月;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.day & 0xf0) << 4) | 0x3000;//解析年的高四为日的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.day & 0x0f) << 8) | 0x3000;//解析年的高四为日的个位;
	*UCS++ = DAY;// 日;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.hour & 0xf0) << 4) | 0x3000;//解析年的高四为日的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.hour & 0x0f) << 8) | 0x3000;//解析年的高四为日的个位;
	*UCS++ = HOUR;// 时;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.minute & 0xf0) << 4) | 0x3000;//解析年的高四为日的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.minute & 0x0f) << 8) | 0x3000;//解析年的高四为日的个位;
	*UCS++ = MINUTE;// 分;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.second & 0xf0) << 4) | 0x3000;//解析年的高四为日的十位;
	*UCS++ = ((uint16_t )(sms_alarm_mail->time.second & 0x0f) << 8) | 0x3000;//解析年的高四为日的个位;
	*UCS++ = SECOND;// 秒;
	*UCS_Len += 20;
	*UCS++ = FAN;
	*UCS++ = SHENG;
